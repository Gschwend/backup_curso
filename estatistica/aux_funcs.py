...
def plot_fit_degenerated(x, y, err,
    a_vals, b_vals, fit_ind, func=func):
    use = x!=x
    use[fit_ind] = True
    plot_data = {
        'datapoints':[True for i in range(5)],
    ...
    axes[1].scatter(x[~use], y[~use], color='.6')
    axes[1].errorbar(x[~use], y[~use], err[~use]*plot_data['err']['v'],
                ls='', fmt='.', lw=1, capsize=5, color='.6')
    b = (y-a*x)[use][0]
    axes[1].scatter(x[use], y[use])
    axes[1].errorbar(x[use], y[use], err[use]*plot_data['err']['v'],
                ls='', fmt='.',
                lw=1,
                capsize=5)
